...
NeuralNetwork<float>* createMnistNeuralNetwork(vector<unsigned int> neuronsPerLayer) {
    ...
    NeuralNetwork<float>* nn = new NeuralNetwork<float>(layersNumber, neuronsPerLayer, inputSize, ACTIVATION::RELU, OUTPUT_AND_LOSS::SOFTMAX_CCE);
...
void trainNeuralNetwork(NeuralNetwork<float>* nn, float* trainingDataset, float* labels, unsigned int trainingInputsNumber, float learningRate, unsigned int epochs) {
    vector<float*> tiledTrainingDataset;
    tiledTrainingDataset.push_back(trainingDataset);
    vector<float*> tiledLabels;
    tiledLabels.push_back(labels);
    vector<unsigned int> tiledInputsNumber;
    tiledInputsNumber.push_back(trainingInputsNumber);
    nn->train(&tiledTrainingDataset, &tiledLabels, tiledInputsNumber, epochs, learningRate);
}
...
int main()
{
    ...
    string modelPath = "nn_states/NeuralNetworkDumpFile_l5_i784_200_200_200_100_10_e800.nn";
    ...
    const unsigned int inputSize = 28 * 28; // size of input layer = number of pixels in an image
    const unsigned int outputSize = 10;     // size of output layer = number of possible digits
    const unsigned int trainingInputsNumber = 6000;    // max 60000
    ...
    if (trainModel) {
        saveNeuralNetwork(nn, string("auto"));
    }
